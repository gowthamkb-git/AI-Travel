...
def _score_google_place(item: dict) -> float:
...
def _build_google_place(item: dict) -> Optional[Dict]:
    ...
    return {
        ...
        "_score": _score_google_place(item),
        "_reviews": int(item.get("user_ratings_total", 0) or 0),
    }


def _google_places(location: str, lat: float, lon: float, limit: int = 5) -> List[dict]:
    ...
    try:
        ...
        for query in queries:
            ...
            for item in response.json().get("results") or []:
                place = _build_google_place(item)
                if not place:
                    continue
...
def _search_place_names(location: str, limit: int = 6) -> List[dict]:
    search_terms = [
...
def search_place_names(location: str, limit: int = 6) -> List[dict]:
    return _search_place_names(location, limit)


def get_place_suggestions(lat: float, lon: float, location: str = "", limit: int = 5) -> List[dict]:
    google_places = _google_places(location, lat, lon, limit)
    if google_places:
